...
const sources = {
  'style.css': () => readLocal('style.css'),
  ...Object.fromEntries(Object.keys(THEMES).map(name =>
    [`theme-${name}.css`, () => readLocal(`themes/${name}.css`)])),
  'diagrams.js': () => readLocal('diagrams.js'),
  'tables.js': () => readLocal('tables.js'),
  'page.js': () => readLocal('page.js'),
  'mermaid.js': () =>
    elkOptions(fs.readFileSync(resolve('mermaid/dist/mermaid.min.js'), 'utf8')),
  'katex.css': () => inlineFonts(resolve('katex/dist/katex.min.css')),
  'highlight.css': () =>
    fs.readFileSync(resolve('highlight.js/styles/github.min.css'), 'utf8'),
...
function readAsset(name) {
  ...
  const prebuilt = local(`assets/${name}`);
  return fs.existsSync(prebuilt)
    ? fs.readFileSync(prebuilt, 'utf8')
    : buildAsset(name);
}
...
function readFont(file) {
  if (sea.isSea?.()) {
    try {
      return Buffer.from(sea.getAsset(`fonts/${file}`));
    } catch {
      return null;
    }
  }

  const prebuilt = local(`assets/fonts/${file}`);
...
function elkOptions(code) {
  if (code.split(ELK_ROOT).length !== 2) {
    throw new Error('mermaid.min.js: не найдены параметры корня ELK');
  }
...
function inlineFonts(cssPath) {
  ...
  return css.replace(/src:([^;}]+)/g, (decl, list) => {
...
// Путь относительно этого модуля: в бандле — относительно бандла.
function local(file) {
  return fileURLToPath(new URL(file, import.meta.url));
}

function readLocal(file) {
  return fs.readFileSync(local(file), 'utf8');
}
